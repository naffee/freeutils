...
import { Volume2, Loader2, Download, RotateCcw } from 'lucide-react';

export function VideoVolumeChanger() {
    ...
    const handleProcess = async () => {
        ...
        const outputName = `volume_output.${outputExt}`;
        ...
        try {
            ...
            alert("Failed to change volume. See console for details.");
        } finally {
    ...
    if (!videoUrl) {
        return (
            ...
            <div className="seo-writeup">
                <h2>Adjust Video Volume</h2>
                <p>Increase or decrease the audio volume level of your video. Make sure your content sounds exactly right.</p>
            </div>
                <Dropzone onFileSelect={handleFileSelect} accept="video/*" title="Drop a video to Adjust Volume" />
            </div>
        );
    }
                    ...
                    <div style={{ flex: 1.5, display: 'flex', flexDirection: 'column', background: '#f8fafc', padding: '1.5rem', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                        ...
                        {outputUrl && (
                            <p style={{ textAlign: 'center', marginTop: '1rem', color: '#16a34a', fontWeight: 600, fontSize: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                                <Volume2 size={16} /> Volume changed successfully
                            </p>
                        )}
                    </div>
                    ...
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        ...
                        <div style={{ background: '#ffffff', padding: '1.25rem', borderRadius: '12px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                            ...
                            <div className="control-group">
                                <label style={{ fontSize: '0.85rem', fontWeight: 600, color: '#334155', display: 'flex', justifyContent: 'space-between' }}>
                                    Volume Percentage: <span>{volume}%</span>
                                </label>
                                <input
                                    type="range"
                                    min="0"
                                ...
                                <p style={{ margin: 0, fontSize: '0.75rem', color: '#64748b' }}>
                                    Adjust volume from 0% (mute) to 500% (boost).
                                </p>
                            </div>
                        ...
                        <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            {outputUrl ? (
                                <><a
                                    href={outputUrl}
                                    download={`volume_${videoFile?.name || 'video'}`}
                                ...
                                <button className="btn-primary" onClick={handleProcess} disabled={isProcessing} style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem' }}>
                                    {isProcessing ? <><Loader2 size={16} className="spin" /> Processing Audio...</> : <><Volume2 size={16} /> Change Volume Now</>}
                                </button>
                            )}
